...
#define SYSTEM_PERIPHERAL_CLOCK (50000000/2)
#pragma message "This module requires a definition for the peripheral clock frequency.  Assuming 25MHz Fcy (50MHz Fosc).  Define value if this is not correct."
#endif
...
#define LCD_DATA_MASK                   0b0000111100000000
...
bool LCD_Initialize ( void )
{
    ...
    LCD_DATA_TRIS &= ~LCD_DATA_MASK; /* make bottom 4 bits output */
    LCD_DATA_LAT &= ~LCD_DATA_MASK;  /* clear bottom 4 bits */
...
static void LCD_SendData ( char data )
{
    ...
    int lower_nibble = ((int)(data & 0x000F));   
    int upper_nibble = ((int)((data >> 4) & 0x000F));
    
    LCD_RWSignal_Clear ( ) ;
    LCD_RSSignal_Set ( ) ;
    LCD_DATA_LAT &= ~LCD_DATA_MASK ;
    LCD_DATA_LAT |= (upper_nibble << LCD_DATA_SHIFT) ;
    ...
    LCD_DATA_LAT &= ~LCD_DATA_MASK ;
    LCD_DATA_LAT |= (lower_nibble << LCD_DATA_SHIFT) ;
    LCD_Wait( LCD_SIGNAL_TIMING);
    LCD_EnableSignal_Set ( ) ;
    LCD_Wait( LCD_SIGNAL_TIMING);
    LCD_EnableSignal_Clear ( ) ;
...
{
    //Split data into nibbles 
    int lower_nibble = ((int)(command & 0x000F));   
    int upper_nibble = ((int)((command >> 4) & 0x000F));
