...
//	#define	APPLICATION_CIRCUIT_950				//enable this if Application circuit is built for supporting 950-960 MHz instead of 860-870
    ...
    **********************************************************************/
    ...
    #define FCRCREG     0x3E00
        ...
        #define TX_PACKET_SIZE RADIO_BUFFER_SIZE    //(TX_BUFFER_SIZE+PROTOCOL_HEADER_SIZE+BLOCK_SIZE+MY_ADDRESS_LENGTH+MY_ADDRESS_LENGTH+9)
        #define RX_PACKET_SIZE RADIO_BUFFER_SIZE    //(RX_BUFFER_SIZE+PROTOCOL_HEADER_SIZE+MY_ADDRESS_LENGTH+MY_ADDRESS_LENGTH+BLOCK_SIZE+9)
    #else
        #define TX_PACKET_SIZE  RADIO_BUFFER_SIZE //(TX_BUFFER_SIZE+PROTOCOL_HEADER_SIZE+MY_ADDRESS_LENGTH+MY_ADDRESS_LENGTH+4)
        #define RX_PACKET_SIZE  RADIO_BUFFER_SIZE //(RX_BUFFER_SIZE+PROTOCOL_HEADER_SIZE+MY_ADDRESS_LENGTH+MY_ADDRESS_LENGTH+4)
    #endif


    #if RX_PACKET_SIZE > RADIO_BUFFER_SIZE
        #warning  "Maximum RX PACKET SIZE should not be greater RADIO_BUFFER_SIZE, redefine RX BUFFER SIZE"
        #undef RX_PACKET_SIZE
        #define RX_PACKET_SIZE RADIO_BUFFER_SIZE
    #endif
    
    #if TX_PACKET_SIZE > RADIO_BUFFER_SIZE
        #warning  "Maximum TX PACKET SIZE should not be greater RADIO_BUFFER_SIZE, redefine TX BUFFER SIZE"
        #undef TX_PACKET_SIZE
        #define TX_PACKET_SIZE RADIO_BUFFER_SIZE
    #endif
    ...
    typedef struct
    {
        ...
        uint8_t        Payload[RADIO_BUFFER_SIZE];
        uint8_t        PayloadLen;
        uint8_t         RSSI;
        uint32_t        PacketTimeMs;
    } RX_PACKET;
