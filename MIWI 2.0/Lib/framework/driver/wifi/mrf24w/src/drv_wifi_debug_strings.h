...
#if (MY_DEFAULT_PS_POLL == WF_ENABLED)
#error "MY_DEFAULT_PS_POLL must be set to WF_DISABLED when running console demo in Ad-Hoc mode (see drv_wifi_config.h)"
#endif

#if defined(ENABLE_DHCP_IP)
#error "To start up in Ad-Hoc mode, comment out ENABLE_DHCP_IP and define ENABLE_STATIC_IP"
#endif

#if !defined(STACK_USE_DHCP_SERVER)
#error "To start up in Ad-Hoc mode, please define STACK_USE_DHCP_SERVER"
#endif
#endif
...
// WPS is only supported by MRF24WG */
...
#error "The only security modes allowed in Ad-Hoc mode are WF_OPEN, WF_SECURITY_WEP_40, or WF_SECURITY_WEP_104"
#endif

#if (MY_DEFAULT_LIST_RETRY_COUNT == WF_RETRY_FOREVER)
#error "For Ad-Hoc connections do not set MY_DEFAULT_LIST_RETRY_COUNT to WF_RETRY_FOREVER.  Setting it to 3 is recommended"
#endif
#endif
