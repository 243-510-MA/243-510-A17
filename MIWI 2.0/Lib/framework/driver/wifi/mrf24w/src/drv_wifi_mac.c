...
static bool   s_wasDiscarded;

static uint8_t  s_encPtrRAWId[2];  /* indexed by ENC_RD_PTR_ID (0) and ENC_WT_PTR_ID (1).  Values can be:           */
                                   /*   RAW_RX_ID, RAW_TX_ID, BACKE_TCB_ADDR, RAW_INVALID_ID                        */

/* keeps track of ENC read and write indexes */
static uint16_t s_encIndex[2]; /* index 0 stores current ENC read index, index 1 stores current ENC write index */

static uint16_t s_rxBufferSize;
static uint16_t s_txPacketLength;
static bool   s_txBufferFlushed;
static bool   s_mgmtRxInProgress = false;
static bool   s_mgmtAppWaiting = false;
static uint16_t s_sizeofScratchMemory = 0;
 ...
 *                      Values are ENC_RD_PTR_ID, ENC_WT_PTR_ID.  s_encIndex[] must be
...
static void SyncENCPtrRAWState(uint8_t encPtrId)
{
    ...
    /*----------------------------------------------------*/
    if ( s_encIndex[encPtrId] < BASE_SCRATCH_ADDR/*BASE_TCB_ADDR*/ )
    {
        /*--------------------------------------*/
        /* if encPtr[encPtrId] in enc Rx buffer */
        /*--------------------------------------*/
        if ( s_encIndex[encPtrId] < TXSTART )
        {
            /* set the rawId */
            rawId = RAW_RX_ID;

            /* Convert encPtr index to Raw Index */
            rawIndex = s_encIndex[encPtrId] - ENC_RX_BUF_TO_RAW_RX_BUF_ADJUSTMENT;

            // encPtr[encPtrId] < (RXSTART + ENC_PREAMBLE_SIZE) is an error since we don't have
            // the same preamble as the ENC chip
            WF_ASSERT( s_encIndex[encPtrId] >= (RXSTART + ENC_PREAMBLE_SIZE) );
        }
        /*----------------------------------------*/
        ...
        else
        {
            ...
            rawIndex = s_encIndex[encPtrId] - ENC_TX_BUF_TO_RAW_TX_BUF_ADJUSTMENT;

            /* encPtr[encPtrId] < BASE_TX_ADDR is an error since we don't have the same  */
            /* pre-BASE_TX_ADDR or post tx buffer as the ENC chip                        */
            WF_ASSERT((s_encIndex[encPtrId] >= BASE_TX_ADDR) && (s_encIndex[encPtrId] <= (BASE_TX_ADDR + MAX_PACKET_SIZE)));
        }
    ...
    else
    {
        ...
        {
            if ( (s_encPtrRAWId[1 - encPtrId]) == RAW_RX_ID )
            {
        ...
        /* convert Enc ptr index to raw index */
        rawIndex = s_encIndex[encPtrId] - ENC_TCB_BUF_TO_RAW_SCRATCH_BUF_ADJUSTMENT;
    }
    ...
    // mark the s_encPtrRAWId[encPtrId] = RAW_INVALID_ID

    // set the s_encPtrRAWId
    s_encPtrRAWId[encPtrId] = rawId;

    // if the opposite encPtr was pointing to the raw window
    // that was re-configured by this routine then it is
    // no longer in sync
    if ( s_encPtrRAWId[1-encPtrId] == s_encPtrRAWId[encPtrId] )
    {
        s_encPtrRAWId[1-encPtrId] = RAW_INVALID_ID;
    }
    ...
    //    s_encPtrRAWId[encPtrId] = RAW_INVALID_ID;
    //}
}
...
void MACProcess(void)
{
    ...
    if ( s_mgmtAppWaiting )
    {
        if ( GetRawWindowState(RAW_TX_ID) == WF_RAW_DATA_MOUNTED )
        {
            ...
            if ( s_encPtrRAWId[ENC_RD_PTR_ID] == RAW_RX_ID )
            {
                s_encPtrRAWId[ENC_RD_PTR_ID] = RAW_INVALID_ID;
            }

            if ( s_encPtrRAWId[ENC_WT_PTR_ID] == RAW_TX_ID )
            {
                s_encPtrRAWId[ENC_WT_PTR_ID] = RAW_INVALID_ID;
            }
        }
...
void RawInit(void)
{
    ...
    s_sizeofScratchMemory = ScratchMount(RAW_RX_ID);  /* put back in if need to know size of scratch */
    ScratchUnmount(RAW_RX_ID);

    RawWindowReady[RAW_RX_ID] = false;

    s_encPtrRAWId[ENC_RD_PTR_ID] = RAW_RX_ID;
    s_encIndex[ENC_RD_PTR_ID] = BASE_SCRATCH_ADDR;//BASE_TCB_ADDR;
    ...
    s_encPtrRAWId[ENC_WT_PTR_ID] = RAW_INVALID_ID;
    s_encIndex[ENC_WT_PTR_ID] = BASE_TX_ADDR; // set tx encode ptr (index) to start of tx buf + 4 bytes

    s_wasDiscarded    = true;   // set state such that last rx packet was discarded
    s_rxBufferSize    = 0;      // current rx buffer length (none) is 0 bytes
    s_txPacketLength  = 0;      // current tx packet length (none) is 0 bytes
    s_txBufferFlushed = true;   // tx buffer is flushed
...
bool MACIsTxReady(void)
{
    ...
    if ( !RawWindowReady[RAW_TX_ID] )
    {
        ...
        if ( s_encPtrRAWId[ENC_RD_PTR_ID] == RAW_TX_ID )
        {
            s_encPtrRAWId[ENC_RD_PTR_ID] = RAW_INVALID_ID;
        }
...
//        RawWindowReady[RAW_RX_ID] = true;
    ...
    s_mgmtRxInProgress = action;
}

bool isRawRxMgmtInProgress(void)
{
     return s_mgmtRxInProgress;   // RawMgtmAppWaiting flag not needed and was causing problems
}
...
uint16_t MACGetFreeRxSize(void)
{
    ...
    {
        if ( (RXSTOP - RXSTART) > s_rxBufferSize )
        {
            size = (RXSTOP - RXSTART) - s_rxBufferSize;
        }
        else
        {
            size = 0;
...
bool MACGetHeader(MAC_ADDR *remote, uint8_t* type)
{
    ...
    if ( s_encPtrRAWId[ENC_WT_PTR_ID] == RAW_RX_ID )
    {
        s_encPtrRAWId[ENC_WT_PTR_ID] = RAW_INVALID_ID;
    }
    ...
    s_encPtrRAWId[ENC_RD_PTR_ID] = RAW_RX_ID;
    s_encIndex[ENC_RD_PTR_ID]    = RXSTART + sizeof(ENC_PREAMBLE);
...
{
    uint8_t buf[14];
    s_txBufferFlushed = false;
    s_txPacketLength = dataLen + (uint16_t)sizeof(ETHER_HEADER) + WF_TX_PREAMBLE_SIZE;

    // Set the SPI write pointer to the beginning of the transmit buffer (post WF_TX_PREAMBLE_SIZE)
    s_encIndex[ENC_WT_PTR_ID] = TXSTART + WF_TX_PREAMBLE_SIZE;
...
void MACFlush(void)
{
    ...
    WF_ASSERT(!s_txBufferFlushed);

    s_txBufferFlushed = true;
...
{
    s_encIndex[ENC_RD_PTR_ID] = RXSTART + sizeof(ENC_PREAMBLE) + offset;
...
PTR_BASE MACSetWritePtr(PTR_BASE address)
{
    ...
    oldVal = s_encIndex[ENC_WT_PTR_ID];

    s_encIndex[ENC_WT_PTR_ID] = address;
...
PTR_BASE MACSetReadPtr(PTR_BASE address)
{
    ...
    oldVal = s_encIndex[ENC_RD_PTR_ID];

    s_encIndex[ENC_RD_PTR_ID] = address;
    SyncENCPtrRAWState(ENC_RD_PTR_ID);
...
uint16_t MACCalcRxChecksum(uint16_t offset, uint16_t len)
{
    ...
    rdSave = s_encIndex[ENC_RD_PTR_ID];

    s_encIndex[ENC_RD_PTR_ID] = temp;
    SyncENCPtrRAWState(ENC_RD_PTR_ID);

    temp = CalcIPBufferChecksum(len);

    s_encIndex[ENC_RD_PTR_ID] = rdSave;
    SyncENCPtrRAWState(ENC_RD_PTR_ID);
...
void MACMemCopyAsync(PTR_BASE destAddr, PTR_BASE sourceAddr, uint16_t len)
{
    ...
    if ( ((TCPIP_UINT16_VAL*)&destAddr)->bits.b15 )
    {
        ...
        destAddr = s_encIndex[ENC_WT_PTR_ID];
        if ( s_encPtrRAWId[ENC_WT_PTR_ID] == RAW_INVALID_ID )
        {
            SyncENCPtrRAWState(ENC_WT_PTR_ID);
        }
    }
    else
    {
        ...
        writeSave = s_encIndex[ENC_WT_PTR_ID];
        s_encIndex[ENC_WT_PTR_ID] = destAddr;
        SyncENCPtrRAWState(ENC_WT_PTR_ID);
    }
    ...
    if ( ((TCPIP_UINT16_VAL*)&sourceAddr)->bits.b15 )
    {
        ...
        sourceAddr = s_encIndex[ENC_RD_PTR_ID];
        if ( s_encPtrRAWId[ENC_RD_PTR_ID] == RAW_INVALID_ID )
        {
            SyncENCPtrRAWState(ENC_RD_PTR_ID);
        }
    }
    else
    {
        ...
        readSave = s_encIndex[ENC_RD_PTR_ID];
        s_encIndex[ENC_RD_PTR_ID] = sourceAddr;
        SyncENCPtrRAWState(ENC_RD_PTR_ID);
    }
    ...
    else if ( len > 0)
    {
        ...
        RawToRawCopy(s_encPtrRAWId[ENC_WT_PTR_ID], len);
    }

    if ( !updateReadPointer )
    {
        s_encIndex[ENC_RD_PTR_ID] = readSave;
        SyncENCPtrRAWState(ENC_RD_PTR_ID);
    }

    if ( !updateWritePointer )
    {
        s_encIndex[ENC_WT_PTR_ID] = writeSave;
        SyncENCPtrRAWState(ENC_WT_PTR_ID);
    }
...
uint8_t MACGet()
{
    ...
    RawGetByte(s_encPtrRAWId[ENC_RD_PTR_ID], &result, 1);

    s_encIndex[ENC_RD_PTR_ID] += 1;
...
uint16_t MACGetArray(uint8_t *val, uint16_t len)
{
    ...
    if ( val )
    {
        RawGetByte(s_encPtrRAWId[ENC_RD_PTR_ID], val, len);
    }
    else
    {
        // Read the data
        while (i < len)
        {
            RawGetByte(s_encPtrRAWId[ENC_RD_PTR_ID], &byte, 1);
            i++;
        }
    }
    s_encIndex[ENC_RD_PTR_ID] += len;
...
void MACPut(uint8_t val)
{
    ...
    RawSetByte(s_encPtrRAWId[ENC_WT_PTR_ID], &byte, 1);

    s_encIndex[ENC_WT_PTR_ID] += 1;
}//end MACPut
...
void MACPutArray(uint8_t *val, uint16_t len)
{
    ...
    RawSetByte(s_encPtrRAWId[ENC_WT_PTR_ID], val, len);

    s_encIndex[ENC_WT_PTR_ID] += len;
}//end MACPutArray
...
void MACPutROMArray(ROM uint8_t *val, uint16_t len)
{
    ...
    RawSetByteROM(s_encPtrRAWId[ENC_WT_PTR_ID], val, len);
