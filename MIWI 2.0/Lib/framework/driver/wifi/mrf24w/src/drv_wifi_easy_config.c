...
void WFEasyConfigInit()
{
    ...
    CFGCXT.wepIndex = WF_WEP_KEY_INVALID;
    CFGCXT.securityMode = WF_SECURITY_OPEN;
    CFGCXT.securityKey[0] = 0;
...
static int WFEasyConfigProcess(void)
{
    ...
    if (CFGCXT.ssid)
        ...
        if ((uint8_t)CFGCXT.securityMode == WF_SECURITY_WPA_AUTO_WITH_PASS_PHRASE)
        {
            WF_DeriveKey(strlen((char *)CFGCXT.securityKey), CFGCXT.securityKey, strlen((char*)CFGCXT.ssid), CFGCXT.ssid);
            CFGCXT.securityMode--;
#if MY_DEFAULT_NETWORK_TYPE == WF_ADHOC
            if (AppConfig.securityMode == WF_SECURITY_WPA_AUTO_WITH_PASS_PHRASE) {
                WF_DeriveKey(AppConfig.securityKeyLength, AppConfig.securityKey,
                AppConfig.ssidLen, AppConfig.ssid);
    ...
    case WF_SECURITY_WEP_40:
        {
            ...
            if (CFGCXT.securityKey) {
                /* Clear key */
                for (i = 0; i < 20; i++)
                    keys[i] = 0;
                memcpy(keys, (void*)CFGCXT.securityKey, 20);
                WF_CPSetSecurity(ConnectionProfileID, WF_SECURITY_WEP_40, CFGCXT.wepIndex, keys, 20);
            }
        }
        break;
    ...
    case WF_SECURITY_WEP_104:
        {
            ...
            if (CFGCXT.securityKey) {
                ...
                for (i = 0; i < 52; i++)
                    keys[i] = 0;
                memcpy(keys, (void*)CFGCXT.securityKey, 52);
                WF_CPSetSecurity(ConnectionProfileID, WF_SECURITY_WEP_104, CFGCXT.wepIndex, keys, 52);
            }
        }
        break;
    ...
    case WF_SECURITY_WPA_AUTO_WITH_KEY:
        if (CFGCXT.securityKey) {
            WF_CPSetSecurity(ConnectionProfileID, WF_SECURITY_WPA_AUTO_WITH_KEY, 0, CFGCXT.securityKey, 32);
        }
        break;

    case WF_SECURITY_WPA_AUTO_WITH_PASS_PHRASE:
        if (CFGCXT.securityKey) {
            WF_CPSetSecurity(ConnectionProfileID, WF_SECURITY_WPA_AUTO_WITH_PASS_PHRASE, 0, CFGCXT.securityKey, strlen((char *)CFGCXT.securityKey));
        }
        break;
    }
    ...
    /* Set wlan mode */
    WF_CPSetNetworkType(ConnectionProfileID, CFGCXT.networkType);
