...
void WF_CPCreate(uint8_t *p_CpId)
{
    ...
    *p_CpId = 0xff;
    ...
    WaitForMgmtResponseAndReadData(WF_CP_CREATE_PROFILE_SUBTYPE, 1, MGMT_RESP_1ST_DATA_BYTE_INDEX, p_CpId);                       
}
...
void WF_CPDelete(uint8_t CpId)
{
    ...
    SendMgmtMsg(hdr, sizeof(hdr), &CpId, 1);              
...
void WF_CPGetIds(uint8_t *p_cpIdList)
{
    ...
    WaitForMgmtResponseAndReadData(WF_CP_GET_ID_LIST_SUBTYPE, 1, MGMT_RESP_1ST_DATA_BYTE_INDEX, p_cpIdList);                     
}
...
void WF_CPSetElements(uint8_t CpId, tWFCPElements *p_elements)
{
    ...
    LowLevel_CPSetElement(CpId,                   /* CP ID                        */
                          ...
                          g_ModuleVer1209orLater ? sizeof(tWFCPElements)
...
/*******************************************************************************
  ...
  Parameters:
    CpId - Connection Profile ID
    p_cred - Pointer to where the credentials will be written.
...
/*******************************************************************************
  ...
  Parameters:
    CpId - Connection Profile ID
    wepIndex - Use WEP key index 0. No longer supporting 4 WEP key indexes (0 - 3).
...
void WF_CPSetDefaultWepKeyIndex(uint8_t CpId, uint8_t wepIndex)
{
    LowLevel_CPSetElement(CpId,                        /* CP ID                        */
                          ...
                          &wepIndex,                   /* pointer to element data      */
                          1);                          /* number of element data bytes */
}
...
/*******************************************************************************
  ...
  Parameters:
    CpId - Connection Profile ID
    p_wepIndex - Pointer to index of WEP key to use.
...
void WF_CPGetDefaultWepKeyIndex(uint8_t CpId, uint8_t *p_wepIndex)
{
    LowLevel_CPGetElement(CpId,                        /* conn. profile ID       */
                          WF_CP_ELEMENT_WEP_KEY_INDEX, /* element ID             */
                          p_wepIndex,                  /* element data pointer   */
                          1,                           /* read one byte          */
                          true);                       /* read data, free buffer */
}
                                    ...
                                    uint8_t dataReadAction)    /* true or false */
{
    ...
    if (dataReadAction == (uint8_t)true)
    {
        ...
        WaitForMgmtResponseAndReadData(WF_CP_GET_ELEMENT_SUBTYPE, elementDataLength,                   
            sizeof(tCPElementResponseHdr), p_elementData);                      
    }
    else
    {
