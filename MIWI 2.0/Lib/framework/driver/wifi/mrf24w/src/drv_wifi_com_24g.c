...
static uint8_t s_txBuf[3];
static uint8_t s_rxBuf[3];

static uint8_t s_HostIntSaved = 0;
...
static uint8_t  s_EintHostIntRegValue;
static uint8_t  s_EintHostIntMaskRegValue;
static uint8_t  s_EintHostInt;

static bool             s_MgmtReadMsgReady;                  /* true if rx mgmt msg to process, else false              */
static volatile bool    s_ExIntNeedsServicing;               /* true if external interrupt needs processing, else false */

#if defined(WF_USE_POWER_SAVE_FUNCTIONS)
static bool s_EnablePowerSaveMode = false;
#endif

tRawMoveState RawMoveState;
extern bool g_WaitingForMgmtResponse;
extern bool g_ReinforceWep;
...
extern void WF_ReinforceWep(void);
...
void WFProcess(void)
{
    ...
    if (g_ReinforceWep == true)
    {
        g_ReinforceWep = false;
        WF_ReinforceWep();
    }
}
...
static void ProcessInterruptServiceResult(void)
{
    ...
    hostIntRegValue |= s_HostIntSaved;
...
uint8_t Read8BitWFRegister(uint8_t regId)
{
	...
	s_txBuf[0] = regId | WF_READ_REGISTER_MASK;
    WF_SpiRead(s_txBuf, 1, s_rxBuf, 1);
...
void Write8BitWFRegister(uint8_t regId, uint8_t value)
{
    ...
    s_txBuf[0] = regId | WF_WRITE_REGISTER_MASK;
    s_txBuf[1] = value;
    WF_SpiWrite(s_txBuf, 2);
...
uint16_t Read16BitWFRegister(uint8_t regId)
{
    ...
    s_txBuf[0] = regId | WF_READ_REGISTER_MASK;
    WF_SpiRead(s_txBuf, 1, s_rxBuf, 2);

    if (intEnabled) 
        WF_EintEnable();

    return (((uint16_t)s_rxBuf[0]) << 8) | ((uint16_t)(s_rxBuf[1]));
}
...
void Write16BitWFRegister(uint8_t regId, uint16_t value)
{
    ...
    s_txBuf[0] = regId | WF_WRITE_REGISTER_MASK;
    s_txBuf[1] = (uint8_t)(value >> 8);       /* MS byte being written     */
    s_txBuf[2] = (uint8_t)(value & 0x00ff);   /* LS byte being written     */

    WF_SpiWrite(s_txBuf, 3);
...
void WriteWFArray(uint8_t regId, uint8_t *p_Buf, uint16_t length)
{
    ...
    s_txBuf[0] = regId;
    WF_SpiWrite2(s_txBuf, 1, p_Buf, length);
...
void ReadWFArray(uint8_t  regId, uint8_t *p_Buf, uint16_t length)
{
    ...
    s_txBuf[0] = regId | WF_READ_REGISTER_MASK;
    WF_SpiRead(s_txBuf, 1, p_Buf, length);
...
void WFEintHandler(void)
{
    ...
    if (RawMoveState.waitingForRawMoveCompleteInterrupt)
    {
        ...
        s_EintHostIntRegValue      = Read8BitWFRegister(WF_HOST_INTR_REG);
        s_EintHostIntMaskRegValue  = Read8BitWFRegister(WF_HOST_MASK_REG);

        // AND the two registers together to determine which active, enabled interrupt has occurred
        s_EintHostInt = s_EintHostIntRegValue & s_EintHostIntMaskRegValue;
        ...
        if (s_EintHostInt & (WF_HOST_INT_MASK_RAW_0_INT_0 | WF_HOST_INT_MASK_RAW_1_INT_0 | WF_HOST_INT_MASK_INT2))
        {
            /* save the copy of the active interrupts */
            RawMoveState.rawInterrupt = s_EintHostInt;
            ...
            if ((s_EintHostInt & ~(WF_HOST_INT_MASK_RAW_0_INT_0 | WF_HOST_INT_MASK_RAW_1_INT_0 | WF_HOST_INT_MASK_INT2)) == 0)
            {
            ...
            else
            {
                ...
                Write16BitWFRegister(WF_HOST_INTR2_REG, (WF_HOST_INT_MASK_RAW_2_INT_0   |
                                                         ...
                                                         WF_HOST_INT_MASK_RAW_4_INT_0   |
                                                         WF_HOST_INT_MASK_RAW_5_INT_0));
                s_HostIntSaved |= (s_EintHostInt & ~(WF_HOST_INT_MASK_RAW_0_INT_0 | WF_HOST_INT_MASK_RAW_1_INT_0 | WF_HOST_INT_MASK_INT2));
                Write8BitWFRegister(WF_HOST_INTR_REG, s_EintHostInt);
    ...
    s_ExIntNeedsServicing = true;
}

void ReenablePowerSaveMode(void)
{
#if defined(WF_USE_POWER_SAVE_FUNCTIONS)
    s_EnablePowerSaveMode = true;
#endif
}
