...
static uint8_t s_powerSaveState = WF_PS_OFF;
...
#if !defined(MRF24WG)
extern bool g_ModuleVer1209orLater;        /* Applicable for MRF24WB0M only */
#endif
...
#if !defined(MRF24WG)
    if (g_ModuleVer1209orLater && !WF_isConnected())
...
bool WFisPsPollEnabled(void)
{
    if ((s_powerSaveState == WF_PS_PS_POLL_DTIM_ENABLED) || (s_powerSaveState == WF_PS_PS_POLL_DTIM_DISABLED))
    {
        return true;
    }
    else
...
static void SendPowerModeMsg(tWFPwrModeReq *p_powerMode)
{
    ...
    SendMgmtMsg(hdr, sizeof(hdr), (uint8_t *)p_powerMode, sizeof(tWFPwrModeReq));
