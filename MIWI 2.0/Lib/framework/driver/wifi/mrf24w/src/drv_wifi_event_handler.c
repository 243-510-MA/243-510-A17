...
void WFProcessMgmtIndicateMsg()
{
    ...
    if (isNotifyApp(event))
    {
        ...
        if (AppConfig.networkType == WF_INFRASTRUCTURE &&
            (AppConfig.securityMode == WF_SECURITY_WEP_40 || AppConfig.securityMode == WF_SECURITY_WEP_104) &&
            (event == WF_EVENT_CONNECTION_SUCCESSFUL || event == WF_EVENT_CONNECTION_REESTABLISHED))
        {
            g_ReinforceWep = true;
        }
    }
...
void WF_ReinforceWep(void)
{
    ...
    data[0] = 1;
    data[1] = 1;
    SendSetParamMsg(PARAM_SECURITY_CONTROL, (uint8_t *)data, 2);
#endif
}
