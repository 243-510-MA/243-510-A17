...
void WF_OutputConnectionInfo(const APP_CONFIG *p_AppConfig)
{
    ...
    putrsUART("SSID:            ");
    if (p_AppConfig->ssid[0] == '\0')
    {
        putrsUART("(none)");
    }
    else
    {
        putsUART((char *)p_AppConfig->ssid);
    }
    putrsUART("\r\n");
   ...
   putrsUART("Active Scan\r\n");
    ...
    WF_OutputSecurityDebugMessage(AppConfig.wepIndex);
...
static void WF_OutputSecurityDebugMessage(uint8_t wepKeyIndex)
{
    ...
    case WF_SECURITY_WPA_AUTO_WITH_KEY:
        if (AppConfig.passphraseToBinaryKeyFlag == 0)
    ...
    case WF_SECURITY_WPA_WITH_KEY:
        if (AppConfig.passphraseToBinaryKeyFlag == 0)
            putrsUART("WPA-PSK with key\r\n");
        else
            putrsUART("WPA-PSK with passphrase\r\n");
        break;
    ...
    case WF_SECURITY_WPA2_WITH_KEY:
        if (AppConfig.passphraseToBinaryKeyFlag == 0)
            putrsUART("WPA2-PSK with key\r\n");
        else
            putrsUART("WPA2-PSK with passphrase\r\n");
        break;
...
void WF_AssertionFailed(uint8_t moduleNumber, uint16_t lineNumber)
{
...
#if defined(USE_LCD)
    {
        ...
        tcpip_helper_uitoa(moduleNumber, (uint8_t*)buf);
        memcpy((void *)&LCDText[18], (void *)buf, strlen(buf));
        LCDText[23] = 'L';
        LCDText[24] = ':';
        tcpip_helper_uitoa(lineNumber, &LCDText[25]);
        LCDUpdate();
    }
#endif
