...
static ROM int8_t s_CmdLinePrompt[] = "> ";
static  uint8_t s_CmdLinePromptLength = 2;
static  int8_t s_TmpCmdLine[kConsoleMaxMsgSize];
...
static void InsertCharacter(int8_t c)
{
   ...
   len = GET_LEN_RX_CMD_STRING() + s_CmdLinePromptLength;

   /* if inserting a character at end of cmd line */
   if (GET_CURSOR() == len)
   {
      g_ConsoleContext.rxBuf[GET_CURSOR() - s_CmdLinePromptLength] = c;
   ...
   else
   {
      ...
      memset(s_TmpCmdLine, 0x00, sizeof(s_TmpCmdLine));

      /* copy up to the point of insertion */
      strncpy( (char *) s_TmpCmdLine, (const char *) g_ConsoleContext.rxBuf, GET_CURSOR() - s_CmdLinePromptLength);

      /* insert the new character */
      s_TmpCmdLine[GET_CURSOR() - s_CmdLinePromptLength] = c;

      /* copy the chars after the new character */
      strncpy( (char *) &s_TmpCmdLine[GET_CURSOR() - s_CmdLinePromptLength + 1],
               (const char *) &g_ConsoleContext.rxBuf[GET_CURSOR() - s_CmdLinePromptLength],
               len - GET_CURSOR());

      /* put the first part of new string in the cmd line buffer */
      strcpy( (char *) g_ConsoleContext.rxBuf, (const char *) s_TmpCmdLine);

      /* erase entire line, put the cursor at index 0 */
      EraseEntireLine();

      /* output the prompt */
      putrsUART( (ROM FAR char *) s_CmdLinePrompt);
...
static void Delete(void)
{
   ...
   if (GET_CURSOR() != GET_LEN_RX_CMD_STRING() + s_CmdLinePromptLength)
   {
      /* Null out tmp cmd line */
      memset(s_TmpCmdLine, 0x00, sizeof(s_TmpCmdLine));

      /* get characters before the deleted key */
      num_chars = GET_CURSOR() - s_CmdLinePromptLength;
      strncpy( (char *) s_TmpCmdLine, (const char *) g_ConsoleContext.rxBuf, num_chars);

      /* append characters after the deleted char (if there are any) */
      if (strlen( (char *) g_ConsoleContext.rxBuf) - 1 > 0u)
      {
         strcpy( (char *) &s_TmpCmdLine[num_chars], (const char *) &g_ConsoleContext.rxBuf[num_chars + 1]);
      }
      ...
      SET_CURSOR(s_CmdLinePromptLength + GET_LEN_RX_CMD_STRING());
      CursorHome(); /* to first character after prompt */

      /* move cursor to point of delete */
      CursorRight_N(orig_index - s_CmdLinePromptLength);
   }
}
...
static void DisplayHistoryEntry(uint8_t action)
{
   ...
   if (foundEntry)
   {
      ...
      SET_CURSOR(s_CmdLinePromptLength + strlen( (char *) history.buf[history.recallIndex]));
   }
...
static void Backspace(void)
{
   ...
   if (GET_CURSOR() != s_CmdLinePromptLength)
   {
      /* Null out tmp cmd line */
      memset(s_TmpCmdLine, 0x00, sizeof(s_TmpCmdLine));

      /* get characters before the backspace */
      num_chars = GET_CURSOR() - s_CmdLinePromptLength - 1;
      strncpy( (char *) s_TmpCmdLine, (const char *) g_ConsoleContext.rxBuf, num_chars);
      ...
      CursorRight_N(orig_index - 1 - s_CmdLinePromptLength);
   }
}
...
static void OutputCommandPrompt(void)
{
    ...
    SET_CURSOR(s_CmdLinePromptLength);
    memset(g_ConsoleContext.rxBuf, 0x00, sizeof(g_ConsoleContext.rxBuf));
...
void CursorLeft(void)
{
   ...
   if (GET_CURSOR() > strlenpgm( (ROM FAR char *) s_CmdLinePrompt))
   {
      SET_CURSOR( GET_CURSOR() - 1);
      putrsUART( (ROM FAR char *) cursorLeftEscapeSequence);
   }
...
static void CursorHome(void)
{
   ...
   if (GET_CURSOR() != s_CmdLinePromptLength)
   {
      ...
      CursorLeft_N(GET_CURSOR() - s_CmdLinePromptLength);
   }
}
...
static void CursorEnd(void)
{
   ...
   if ( (GET_LEN_RX_CMD_STRING() + s_CmdLinePromptLength) != GET_CURSOR())
   {
      len = GET_LEN_RX_CMD_STRING() - GET_CURSOR() + s_CmdLinePromptLength;
      CursorRight_N(len);
   }
}
