 ...
 * When transferring the entire packet at a transaction is desired, set this value to 0.  
 */
...
bool WF_SpiTx(unsigned char *buf, uint16_t size)
{
    uint8_t buf_dump[2] = {0, 0};
    WFSpiTxRx(buf, size, buf_dump, 0);
	return true;
}
bool WF_SpiWrite(unsigned char *buf, uint16_t size)
{
    bool ret = true;

    if (size > 0) {
        WF_SpiEnableChipSelect();
        ret = WF_SpiTx(buf, size);
...
bool WF_SpiWrite2(unsigned char *p_cmd, uint16_t cmdLen, unsigned char *p_txBuf, uint16_t txLen)
{
    bool ret = true;

    WF_SpiEnableChipSelect();
    ret = WF_SpiTx(p_cmd, cmdLen);
    if (ret && txLen > 0)
        ret = WF_SpiTx(p_txBuf, txLen);
    WF_SpiDisableChipSelect();
...
bool WF_SpiRx(unsigned char *buf, uint16_t size)
{
    uint16_t chunk_size;
    uint16_t read_size;
    uint8_t buf_dump[2] = {0, 0};
...
{
    uint8_t buf_dump[2] = {0, 0};
    WFSpiTxRx(buf_dump, 0, buf, size);
    return true;
}
#endif /*  MAX_SPI_RX_CHUNK_PER_TRANSACTION > 0 */

bool WF_SpiRead(unsigned char *p_cmd, uint16_t cmdLen, unsigned char *p_rxBuf, uint16_t rxLen)
{
    bool ret = true;
