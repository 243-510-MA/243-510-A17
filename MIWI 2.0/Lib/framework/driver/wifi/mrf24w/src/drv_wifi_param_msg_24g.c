...
void WF_MulticastGetConfig(uint8_t filterId, tWFMultiCastConfig *p_config)
{
    ...
    SendMgmtMsg(hdr, sizeof(hdr), &filterId, 1);              

    WaitForMgmtResponseAndReadData(WF_GET_PARAM_SUBTYPE, sizeof(paramData), MSG_PARAM_START_DATA_INDEX, paramData);                 
...
void WF_SetHostKeyDeriveMode(void)
{
    ...
    SendSetParamMsg(PARAM_SET_HOST_DERIVE_KEY, (uint8_t *)&yield, sizeof(yield));
}
...
/*******************************************************************************
  ...
  Summary:
   Enables multicast filter.

  Description:
    This function allows the application to configure up to 16 Multicast address Filters 
    on the MRF24WG0MA/B.
...
void WF_EnableMultiCastFilter(void)
{
    ...
    SendSetParamMsg(PARAM_SET_MULTICAST_FILTER, (uint8_t *)&enable, sizeof(enable));
}
                     ...
                     uint8_t paramDataLength)
{
    ...
    WaitForMgmtResponseAndReadData(WF_GET_PARAM_SUBTYPE, paramDataLength, MSG_PARAM_START_DATA_INDEX, p_paramData);              
}
