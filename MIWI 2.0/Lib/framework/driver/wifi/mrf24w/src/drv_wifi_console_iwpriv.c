...
static void iwprivDisplayStatus(void)
{
    ...
    // security type
    {
        ...
        case WF_SECURITY_WPA_WITH_KEY:
            WFConsolePrintRomStr("WPA-PSK with Key", true);
            break;
        case WF_SECURITY_WPA_WITH_PASS_PHRASE:
            WFConsolePrintRomStr("WPA-PSK with Passphrase", true);
            break;
        case WF_SECURITY_WPA2_WITH_KEY:
            WFConsolePrintRomStr("WPA2-PSK with Key", true);
            break;
        case WF_SECURITY_WPA2_WITH_PASS_PHRASE:
            WFConsolePrintRomStr("WPA2-PSK with Passphrase", true);
            break;
        default:
    ...
    if ( iwprivCb.securityType == WF_SECURITY_WEP_40 || iwprivCb.securityType == WF_SECURITY_WEP_104 )
    {
        ...
        iwprivCb.securityType == WF_SECURITY_WPA_WITH_KEY || 
        iwprivCb.securityType == WF_SECURITY_WPA2_WITH_KEY)
    {
        ...
        iwprivCb.securityType == WF_SECURITY_WPA_WITH_PASS_PHRASE ||
        iwprivCb.securityType == WF_SECURITY_WPA2_WITH_PASS_PHRASE)
    {
...
static bool iwprivSetEnc(void)
{
    ...
    else if ( (3u <= ARGC) && (strcmppgm2ram((char *)ARGV[2], "WPA-PSK/WPA2-PSK with KEY") == 0) )
    {
        securityType = WF_SECURITY_WPA_AUTO_WITH_KEY;
    }
    else if ( (3u <= ARGC) && (strcmppgm2ram((char *)ARGV[2], "WPA-PSK/WPA2-PSK with Passphrase") == 0) )
    {
