...
{
    return s_encPtrRAWId[ENC_RD_PTR_ID];
}
...
/*****************************************************************************
  ...
  Description:
    ...
    This function is typically called right after s_encIndex[] is updated.
...
static void SyncENCPtrRAWState(uint8_t encPtrId, uint16_t encIndex)
{
    ...
    /*----------------------------------------------------------------------------------*/
    else if (s_encIndex[encPtrId] < BASE_SCRATCH_ADDR/*BASE_TCB_ADDR*/)
    {
...
static void WF_SaveWPSCredentials(uint8_t CpId)
{
    ...
    if (!once) {
        ...
        memcpy((void *)AppConfig.ssid, (void *)cred.ssid, cred.ssidLen);
        AppConfig.ssidLen = cred.ssidLen;
        ...
        case AUTH_OPEN:
            if (cred.encType == ENC_NONE) {
                ...
                if (AppConfig.securityMode == WF_SECURITY_WEP_40) {
                    ...
                    AppConfig.wepIndex = key.wep40.key_idx;
                    ...
                    AppConfig.wepIndex = key.wep104.key_idx;
                } else {
                    WF_ASSERT(false);
                }
            }
...
void RawInit(void)
{
    ...
    s_sizeofScratchMemory = ScratchMount(RAW_SCRATCH_ID);

    s_encPtrRAWId[ENC_RD_PTR_ID] = RAW_DATA_RX_ID;
    s_encIndex[ENC_RD_PTR_ID] = BASE_SCRATCH_ADDR;//BASE_TCB_ADDR;

    SetRawWindowState(RAW_DATA_TX_ID, WF_RAW_UNMOUNTED);

    s_encPtrRAWId[ENC_WT_PTR_ID] = RAW_DATA_TX_ID;
...
void MACMemCopyAsync(PTR_BASE destAddr, PTR_BASE sourceAddr, uint16_t len)
{
    ...
    if ( ((TCPIP_UINT16_VAL*)&destAddr)->bits.b15 )
    {
        ...
        destAddr = s_encIndex[ENC_WT_PTR_ID]; /* don't need to sync write pointer, should have already been done */
    ...
    if ( ((TCPIP_UINT16_VAL*)&sourceAddr)->bits.b15 )
    {
        ...
        sourceAddr = s_encIndex[ENC_RD_PTR_ID]; /* don't need to sync read pointer, should have already been done */
                                                /*  when encIndex was set previously                              */
    }
    ...
    else
    {
        ...
        if (((s_encIndex[ENC_WT_PTR_ID] >= TXSTART) && (s_encIndex[ENC_RD_PTR_ID] >= TXSTART))
                                                    &&
            ((s_encIndex[ENC_WT_PTR_ID] < BASE_SCRATCH_ADDR) && (s_encIndex[ENC_RD_PTR_ID] < BASE_SCRATCH_ADDR)))
        {
        ...
        RawToRawCopy(s_encPtrRAWId[ENC_WT_PTR_ID], s_encPtrRAWId[ENC_RD_PTR_ID], len);
    }
