...
static bool s_RestoreRxData = false;
uint8_t g_WaitingForMgmtResponse = false;
static uint8_t s_FuncFlags = 0x00;
                 ...
                 uint8_t dataLength)
{
    ...
    WF_ASSERT(!((s_FuncFlags & WF_PROCESS_EVENT_FUNC) > 0));
...
void WaitForMgmtResponse(uint8_t expectedSubtype, uint8_t freeAction)
{
    ...
    if (freeAction == FREE_MGMT_BUFFER)
    {
        ...
        if (s_RestoreRxData == true)
        {
            s_RestoreRxData = false;
