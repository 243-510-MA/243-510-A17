...
#define ENABLE_WPS_TRACE_INFORM    (1 << 0)
#define ENABLE_P2P_TRACE_INFORM    (1 << 1)
...
typedef struct WFCPElementsStruct
{
    ...
    /**
      ...
      <table>
        ...
        WF_SECURITY_WPA_WITH_KEY           Use WPA-PSK security.
                                                 32-byte PSK (Pre-shared Key) will be provided in securityKey.
                                                 It is often displayed as 64 hexadecimal characters.
        WF_SECURITY_WPA_WITH_PASSPHRASE    Use WPA-PSK security.
                                                 ASCII passphrase (8 to 63 characters long) will be provided in securityKey.
                                                 MRF24W will convert this to PSK (which can take up to 30 seconds).
        WF_SECURITY_WPA2_WITH_KEY           Use WPA2-PSK security.
                                                 32-byte PSK (Pre-shared Key) will be provided in securityKey.
                                                 It is often displayed as 64 hexadecimal characters.
        WF_SECURITY_WPA2_WITH_PASSPHRASE    Use WPA2-PSK security.
                                                 ASCII passphrase (8 to 63 characters long) will be provided in securityKey.
                                                 MRF24W will convert this to PSK (which can take up to 30 seconds).
      </table>
...
#define WF_ASSERT(expr) do { if (!(expr)) WF_AssertionFailed(WF_MODULE_NUMBER, __LINE__); } while (0)
#else /* !WF_DEBUG */
#define WF_ASSERT(expr) do { if (!(expr)) Reset(); } while (0)
#endif  /* WF_DEBUG */
...
/*******************************************************************************
  ...
  Parameters:
    p_ctx -- pointer to where connection context is returned.
...
void WF_CPSetDefaultWepKeyIndex(uint8_t CpId, uint8_t wepIndex);
void WF_CPGetDefaultWepKeyIndex(uint8_t CpId, uint8_t *p_wepIndex);
...
/*******************************************************************************
  ...
  Parameters:
    dtimInterval -- Number of DTIM intervals.
...
/*******************************************************************************
  ...
  Parameters:
    p_dtimInterval -- pointer to where dtim interval is returned
...
/*******************************************************************************
  ...
  Summary:
    Sets the beacon period in Adhoc mode.

  Description:
    This function sets the beacon period used by the Connection Algorithm.

  Precondition:
    MACInit must be called first.

  Parameters:
    beaconPeriod - beacon period (ms).
...
/*******************************************************************************
  ...
  Summary:
    Gets current beacon period in Adhoc mode.

  Description:
    This function gets current beacon period in Adhoc mode.
...
/*******************************************************************************
  ...
  Description:
    This function sets the MRF24WG Tx mode.
        ...
        mode                           Description
        ----                           -----------
        WF_TXMODE_G_RATES        Use all 802.11g rates (default)
        WF_TXMODE_B_RATES        Use only 802.11b rates
        WF_TXMODE_LEGACY_RATES   Use only 1 and 2 mbps rates
        </table>
...
/*******************************************************************************
  ...
  Parameters:
    p_mode -- Pointer to where mode will be written. 
...
/*******************************************************************************
  ...
  Description:
    ...
    before the connection failure event (WF_LINK_LOST) is reported to the host application.
...
/*******************************************************************************
  ...
  Description:   
   ...
   factory-set value.  This function can set a different maximum Tx power levels.  
   However, this function can never set a maximum Tx power
...
/*******************************************************************************
  Function:
    WF_TxPowerGetMax(int8_t *p_maxTxPower)

  Summary:
    Gets max Tx power.

  Description:
    Get max Tx power currently set on the module.

  Precondition:
    MACInit must be called first.

  Parameters:
    p_maxTxPower - Pointer where the tx power is returned. 
...
/*******************************************************************************
  ...
  Summary:
    Fixes Tx rate with maximum power.

  Description:
   This function fixes Tx rate with maximum power.
...
/*******************************************************************************
  ...
  Summary:
    Gets the factory-set max Tx power (dBm).
  ...
  Parameters:
    p_factoryMaxTxPower - pointer where the value is returned. 
  ...
  *****************************************************************************/
void WF_TxPowerGetFactoryMax(int8_t *p_factoryMaxTxPower);
#endif /* defined(WF_USE_TX_POWER_CONTROL_FUNCTIONS) */
...
#endif /* defined(WF_USE_POWER_SAVE_FUNCTIONS) */
...
/*******************************************************************************
  Function:
    void WF_SetHostKeyDeriveMode(void)
...
/*******************************************************************************
  Function:
    void WF_EnableMultiCastFilter(void)
...
#if defined(MRF24WG)
void WF_DumpModuleAssertInfo(void);
#endif

/*******************************************************************************
  Function:
    void WF_SetReconnectMode(void)

  Summary:
    Set MRF24W's reconnection mode.
  ...
  Remarks:
    None.
 *****************************************************************************/
extern void WF_SetReconnectMode(bool disableConnMgr);
...
extern void WF_DeriveKey(uint8_t key_len, uint8_t *key, uint8_t ssid_len, uint8_t *ssid);

extern bool WF_isConnected(void);
