    ...
    *****************************************************************************************/    
    ...
    bool MiApp_UnicastStar  (uint8_t EndDevice_ConnectionIndex , bool SecEn)
    {
		...
		for (i=0;i<datalen;i++)
		{
            ...
            t1 = MiWi_TickGet();   // Get current time
            while (!rec_ack)
            {
                t2 = MiWi_TickGet();   // Get current time
                MiApp_MessageAvailable(); // False Input parameter as we co ordinator is trying to send Data
                //do
